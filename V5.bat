@echo off
title ***Group核磁数据下载V5 By Kaisoku
echo 你好，%username%！
echo 本程序更新日期：2020/12/22      预祝大家新年快乐！
echo.
echo 使用说明：
echo 本程序会在桌面建立“NMR”文件夹，并按你的需要将Bruker服务器上以你名字缩写开头的核磁数据同步到以你名字缩写命名的文件夹里面。
echo 运行时间取决于数据量、网络质量和硬盘写入速度，通常可以在数分钟内完成。
...
:1
for /d %%a in (%WayIn%\%Name%-*) do ( xcopy /d/e/y/k %%~a %WayOut%\%Name%\%%~nxa\ )
goto Finish
exit

:2
for /d %%a in (%WayIn%\%Name%-%DATE:~0,4%%DATE:~5,2%*) do ( xcopy /d/e/y/k %%~a %WayOut%\%Name%\%%~nxa\ )
goto Finish
exit

:3
for /d %%a in (%WayIn%\%Name%-%TempDate%-*) do ( xcopy /d/e/y/k %%~a %WayOut%\%Name%\%%~nxa\ )
goto Finish
exit
...
for /d %%a in (%WayIn%\%Name%-%SetDate%-*) do ( xcopy /d/e/y/k %%~a %WayOut%\%Name%\%%~nxa\ )
cls
